...
float sdU( vec3 p, float r, float le, vec2 w )
{
    ...
    p.y = p.y - le;
    float k = max(p.x,p.y);
    vec2 q = vec2( (k<0.0) ? -k : length(max(vec2(p.x,p.y),0.0)), abs(p.z) ) - w;
...
vec2 map( in vec3 pos )
{
    ...
    if( sdBox(pos-vec3(2.0,0.3,-1.0),vec3(0.35,0.3,2.5))<res.x )
    {
      ...
      res = opU( res, vec2( sdRoundCone(   pos-vec3( 2.0,0.15, 0.0), 0.2, 0.05, 0.3 ), 51.7 ) );
      res = opU( res, vec2( sdRoundCone(   pos-vec3( 2.0,0.20, 1.0), vec3(0.1,0.0,0.0), vec3(-0.1,0.35,0.1), 0.15, 0.05), 37.0 ) );
    }

    // bounding box
    if( sdBox(pos-vec3(-2.0,0.3,-1.0),vec3(0.35,0.3,2.5))<res.x )
    {
      res = opU( res, vec2( sdU(          pos-vec3(-2.0,0.10, 1.0), 0.2, 0.3, vec2(0.04,0.04)), 39.4 ) );
    }
